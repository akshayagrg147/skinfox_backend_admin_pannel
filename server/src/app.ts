...
import { calculateCart, FREE_SHIPPING_THRESHOLD_PAISE, inclusiveTaxBreakdown, isValidPincode, STANDARD_SHIPPING_PAISE } from './lib/pricing.js'
...
const MAX_CART_ITEM_QUANTITY = 8
const cartItemSchema = z.object({ productId: z.string().min(1), variantId: z.string().optional(), quantity: z.number().int().min(1).max(MAX_CART_ITEM_QUANTITY) })
...
export function buildApp(): FastifyInstance {
  ...
  const getCart = async (request: any) => {
    ...
    // Older releases accepted quantities up to 50. Normalize those stale cart
    // rows once when they are read so refreshes cannot resurrect an oversized
    // line or carry it into checkout.
    const oversizedItems = cart.items.filter((item: any) => item.quantity > MAX_CART_ITEM_QUANTITY)
    if (oversizedItems.length) {
      await prisma.cartItem.updateMany({ where: { id: { in: oversizedItems.map((item: any) => item.id) } }, data: { quantity: MAX_CART_ITEM_QUANTITY } })
      oversizedItems.forEach((item: any) => { item.quantity = MAX_CART_ITEM_QUANTITY })
    }
  ...
  const delhiveryTrackingUrl = (awb: string | null | undefined) => awb ? `https://www.delhivery.com/track/package/${encodeURIComponent(awb)}` : null
  ...
  routes.post('/api/v1/admin/orders/:id/shipment/book', async (request, reply) => {
    ...
    const hasProviderEtd = providerEtd && !Number.isNaN(providerEtd.valueOf())
    const estimateFrom = input.estimatedDeliveryFrom ? new Date(input.estimatedDeliveryFrom) : hasProviderEtd ? providerEtd : input.estimatedDays ? new Date(Date.now() + input.estimatedDays * 86400000) : null
    const estimateTo = input.estimatedDeliveryTo ? new Date(input.estimatedDeliveryTo) : hasProviderEtd ? providerEtd : input.estimatedDays ? new Date(Date.now() + (input.estimatedDays + 2) * 86400000) : null
    const shipment = await prisma.shipment.create({ data: { orderId: order.id, provider: 'delhivery', providerOrderId: created.providerOrderId, providerShipmentId: created.providerShipmentId, courierId: input.courierId, courierName: input.courierName ?? providerValue(assigned, ['data.response.data.courier_name', 'data.courier_name']), trackingNumber: awb, trackingUrl: delhiveryTrackingUrl(awb), providerStatus: providerStatus(assigned) || 'awb_assigned', status: awb ? 'awb_assigned' : 'booked', estimatedDeliveryFrom: estimateFrom, estimatedDeliveryTo: estimateTo, metadata: { createResponse: created.raw, awbResponse: assigned } as any, packedWeightGrams: input.package.weightGrams, lengthCm: input.package.lengthCm, breadthCm: input.package.breadthCm, heightCm: input.package.heightCm, bookedAt: new Date(), events: { create: { status: awb ? 'awb_assigned' : 'booked', externalId: `book-${request.id}`, payload: { create: created.raw, assign: assigned } as any } } } })
    ...
    if (order.customerId) await prisma.customerNotification.create({ data: { customerId: order.customerId, orderId: order.id, shipmentId: shipment.id, type: 'shipment_booked', title: `Order ${order.orderNumber} has shipped`, body: awb ? `Your ${shipment.courierName ?? 'courier'} tracking number is ${awb}.` : 'Your courier shipment is booked. Tracking will be added shortly.' } }).catch(() => undefined)
    await audit(user, request, 'shipment_book', 'Shipment', shipment.id, null, shipment, 'Delhivery shipment booked')
    if (order.customer?.email) {
      const eta = estimateFrom ? `Estimated delivery: ${estimateFrom.toLocaleDateString('en-IN')}${estimateTo && estimateTo.toISOString() !== estimateFrom.toISOString() ? ` – ${estimateTo.toLocaleDateString('en-IN')}` : ''}.` : 'The courier will confirm an estimated delivery date soon.'
      void email.send({ to: order.customer.email, subject: `Your SkinFox order ${order.orderNumber} is on the way`, html: `<p>Your order <strong>${order.orderNumber}</strong> has been handed to ${shipment.courierName ?? 'the courier'}.</p><p>Tracking number: <strong>${awb ?? 'Pending'}</strong></p><p>${eta}</p>` }).catch(() => undefined)
    }
  ...
  const shipmentAction = async (request: any, reply: any, action: 'pickup' | 'label' | 'cancel' | 'refresh') => {
    ...
    if (action === 'pickup') {
      ...
      const input = z.object({ pickupDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), pickupTime: z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/).optional(), packageCount: z.number().int().positive().max(500).optional() }).parse(request.body ?? {})
      const result = await delhivery.requestPickup(shipment.providerShipmentId, input)
      const requestedPickupTime = input.pickupTime ? (input.pickupTime.length === 5 ? `${input.pickupTime}:00` : input.pickupTime) : '16:00:00'
      const requestedPickupAt = input.pickupDate ? new Date(`${input.pickupDate}T${requestedPickupTime}`) : new Date()
      const pickupScheduledAt = Number.isNaN(requestedPickupAt.valueOf()) ? new Date() : requestedPickupAt
      const updated = await prisma.shipment.update({ where: { id: shipment.id }, data: { status: 'pickup_requested', pickupId: providerValue(result, ['data.response.pickup_id', 'data.pickup_id', 'pickup_id']), pickupScheduledAt, providerStatus: providerStatus(result) || 'pickup_requested', events: { create: { status: 'pickup_requested', externalId: `pickup-${request.id}`, payload: result as any } } } })
    ...
    const updated = await prisma.shipment.update({ where: { id: shipment.id }, data: { status: nextStatus.toLowerCase().replaceAll(' ', '_'), providerStatus: nextStatus, trackingUrl: shipment.trackingUrl ?? delhiveryTrackingUrl(shipment.trackingNumber), lastSyncedAt: new Date(), events: { create: { status: nextStatus.toLowerCase().replaceAll(' ', '_'), externalId: `refresh-${request.id}`, payload: result as any } } } })
    const previousStatus = String(shipment.providerStatus ?? shipment.status ?? '').trim().toLowerCase()
    const currentStatus = nextStatus.trim().toLowerCase()
    if (currentStatus && currentStatus !== previousStatus) {
      const customerId = shipment.order.customerId
      const orderNumber = shipment.order.orderNumber
      if (customerId) await prisma.customerNotification.create({ data: { customerId, orderId: shipment.orderId, shipmentId: shipment.id, type: 'shipment_updated', title: `Shipment update for order ${orderNumber}`, body: `Delhivery status: ${nextStatus}.` } }).catch(() => undefined)
      if (shipment.order.customer?.email) void email.send({ to: shipment.order.customer.email, subject: `Tracking update for your SkinFox order ${orderNumber}`, html: `<p>Your Delhivery shipment <strong>${shipment.trackingNumber}</strong> is now <strong>${nextStatus}</strong>.</p><p><a href="${delhiveryTrackingUrl(shipment.trackingNumber) ?? '#'}">Track shipment</a></p>` }).catch(() => undefined)
    }
