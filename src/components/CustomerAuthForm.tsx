...
export function CustomerAuthForm({ apiAvailable, cartToken, destination, onAuthenticated }: { apiAvailable: boolean; cartToken?: string; destination: 'account' | 'checkout'; onAuthenticated: (response: CustomerAuthResponse) => Promise<void> | void }) {
  ...
  return <div className="customer-auth-form">
    <div className="customer-auth-form__intro">
      ...
      <h2>{title}</h2>
      <p>{mode === 'signup' ? 'A little less to fill in next time. Save delivery details and follow your orders in one place.' : mode === 'reset' ? 'Enter your email and we’ll send you a link to choose a new password.' : destination === 'checkout' ? 'Sign in to continue to delivery and keep your order connected to your account.' : 'Your orders, delivery details and everyday care, all in one place.'}</p>
    </div>
