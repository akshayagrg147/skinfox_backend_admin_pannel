...
export function RoutineQuiz({ open, onClose, onAdd, catalogue = [], finder }: RoutineQuizProps) {
  ...
  const packageTotalLabel = result?.package && result.package.totalPaise > 0 ? formatPrice(result.package.totalPaise / 100) : 'Coming soon'
      ...
      {result.package ? <>
        <section className="care-edit" aria-label={result.package.name}>
          ...
          <div className="care-edit__items">{packageItems.map((item, index) => <article className="care-edit-item" key={item.product.id}>
            ...
            <div className="care-edit-item__price"><strong>{formatProductPrice(item.product)}</strong>{savingsLabel(item.product) && <small>{savingsLabel(item.product)}</small>}{item.product.price === null && <small className="care-edit-item__price-note">This product is not available yet.</small>}<button type="button" aria-label={`${addedIds.includes(item.product.id) ? 'Added' : 'Add'} ${item.product.name} to bag`} onClick={() => addItem(item.product)} disabled={item.product.price === null || addedIds.includes(item.product.id)}>{item.product.price === null ? 'Coming soon' : addedIds.includes(item.product.id) ? 'In bag' : 'Add to bag'} {addedIds.includes(item.product.id) ? <Check size={15} aria-hidden="true" /> : <ArrowRight size={15} aria-hidden="true" />}</button></div>
          </article>)}</div>
