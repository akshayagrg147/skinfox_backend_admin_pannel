import { Check, LockKeyhole, MailCheck } from 'lucide-react'
...
export function CheckoutModal({ open, lines, onClose, onComplete, onCustomerChange, cartToken = '', apiAvailable = true }: { open: boolean; lines: CartLine[]; onClose: () => void; onComplete: () => void; onCustomerChange?: (customer: Customer | null) => void; cartToken?: string; apiAvailable?: boolean }) {
  ...
  const submitCheckout = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (hasPendingPrice) { setError('This selection is not purchasable until a selling price is published in the admin panel.'); return }
  ...
  return <ModalShell open={open} onClose={close} title="Secure SkinFox checkout" className="checkout-modal">
    {stage === 'complete' ? <div className="checkout-success"><span><Check size={26} /></span><p className="eyebrow">Cash on delivery confirmed</p><h2>Your SkinFox edit is confirmed.</h2><p>Your order has been created. You will pay only when the order is delivered.</p><button className="button button--dark" onClick={close}>Return to SkinFox</button></div> : <div className="checkout-grid">
      <div className="checkout-form-content">
        ...
        {stage === 'auth' && <><span className="eyebrow">Secure account sign-in</span><CustomerAuthForm apiAvailable={apiAvailable} destination="checkout" cartToken={cartToken} onAuthenticated={authenticated} /></>}
        {stage === 'address' && <form onSubmit={submitCheckout} noValidate>
          <span className="eyebrow">Step 2 of 2 · delivery and COD</span>
          ...
          <button className="button button--copper checkout-submit" type="submit" disabled={submitting || hasPendingPrice || Boolean(customer && !customer.emailVerified)}><LockKeyhole size={16} /> {submitting ? 'Placing COD order…' : `Place COD order · ${formatPrice(subtotal)}`}</button>
        </form>}
        {stage !== 'address' && stage !== 'auth' && error && <p className="form-error" role="alert">{error}</p>}
      </div>
      <aside className="checkout-summary"><span className="eyebrow">Order summary</span>{lines.map((line) => <div key={line.product.id}><span>{line.product.name} <i>× {line.quantity}</i></span><ProductPrice product={line.product} quantity={line.quantity} compact className="checkout-line-price" /></div>)}<hr /><div><span>Shipping</span><strong>{hasPendingPrice ? 'At launch' : 'Calculated by pincode'}</strong></div><div className="checkout-total"><span>Total</span><strong>{hasPendingPrice ? 'Price pending' : formatPrice(subtotal)}</strong></div></aside>
    </div>}
  </ModalShell>
}
