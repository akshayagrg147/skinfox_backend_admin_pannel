import { ArrowLeft, ArrowRight, Check, ChevronRight, Mail, Minus, Plus, Sparkles } from 'lucide-react'
  ...
  return <section className="product-page shell">
    ...
    <div className="product-page__grid">
      ...
      <div className="product-page__information">
        ...
        <div className="product-page__pricing">{checkingPrice ? <p role="status">Checking current launch access…</p> : error ? <p role="alert">We couldn’t load current availability. Please <a href={productPath(displayProduct)}>try again</a>.</p> : <><ProductPrice product={displayProduct} /><small>{displayProduct.price === null ? 'Join the priority waitlist now; review the final price before you purchase.' : 'Current selling price. Delivery details are shown at checkout.'}</small></>}</div>
        <div className="product-page__purchase">
          ...
          <button className="button button--dark" type="button" disabled={checkingPrice || Boolean(error) || !product} onClick={() => product && onAdd(product, quantity)}>{displayProduct.price === null ? 'Join priority waitlist' : 'Add to bag'} <ArrowRight size={18} /></button>
        </div>
    ...
    <footer className="product-page__footer"><a href="/#shop"><ArrowLeft size={15} /> Back to collection</a><div><a href="/privacy-policy">Privacy policy</a><a href="/terms-and-conditions">Terms & conditions</a></div></footer>
  </section>
}
