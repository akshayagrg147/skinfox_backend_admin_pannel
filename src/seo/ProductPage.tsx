...
export function ProductPage({ product, productSlug, loading = false, error = '', catalogProducts = catalog, waitlist, founderNumber, onAdd, onFindCare }: ProductPageProps) {
  ...
  const founderReveal = founderEligible && (launch.stage === 'founder_reveal' || launch.stage === 'launch')
    ...
    <div className="product-page__grid">
      ...
      <div className="product-page__information">
        ...
        <div className="product-page__pricing">{checkingPrice ? <p role="status">Checking current launch access…</p> : error ? <p role="alert">We couldn’t load current availability. Please <a href={productPath(displayProduct)}>try again</a>.</p> : foundingOpen ? <div className="founder-offer">
          ...
          <span className="founder-offer__mrp">MRP {money(mrpPaise)}</span>
          {founderReveal ? <><span className="founder-offer__eyebrow"><Crown size={15} /> Exclusive to the First {launch.founderCapacity} Members</span><strong><em>Member Launch Price</em> {money(shownPrice!)}</strong><p>Early-member benefit: Save {money(mrpPaise - shownPrice!)}. Your launch price is reserved for a limited time.</p></> : founderAccessLocked ? <><span className="founder-offer__eyebrow"><Crown size={15} /> Member reveal in progress</span><strong><em>Launch Price</em> Reserved for Members</strong><p>Sign in with the account used to join the Founding {launch.founderCapacity}. Public Launch Price {money(launch.launchPricePaise)} follows next.</p></> : <><span className="founder-offer__eyebrow"><Sparkles size={15} /> {launch.foundingClosed ? `Founding ${launch.founderCapacity} is now closed` : 'SkinFox collection'}</span><strong><em>{launch.stage === 'regular' ? 'Regular Price' : 'Launch Price'}</em> {money(shownPrice ?? launch.regularPricePaise)}</strong><p>{launch.stage === 'regular' ? 'The SkinFox collection is now available.' : 'The next chapter of the SkinFox launch is now open.'}</p></>}
        </div>}</div>
        ...
        <div className="founder-journey" aria-label="SkinFox launch pricing journey"><span className={launch.stage === 'waitlist' ? 'is-current' : ''}><small>Waitlist</small><strong>Price reserved</strong></span><i /><span className={launch.stage === 'founder_reveal' ? 'is-current' : ''}><small>Founding {launch.founderCapacity}</small><strong>{launch.stage === 'waitlist' ? 'Reveals soon' : money(launch.founderPricePaise ?? 59900)}</strong></span><i /><span className={launch.stage === 'launch' ? 'is-current' : ''}><small>Launch</small><strong>{money(launch.launchPricePaise)}</strong></span><i /><span className={launch.stage === 'regular' ? 'is-current' : ''}><small>Regular</small><strong>{money(launch.regularPricePaise)}</strong></span></div>
