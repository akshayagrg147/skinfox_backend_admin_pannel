...
export function SiteRouter() {
  ...
  const guideMatch = path.match(/^\/guides\/([a-zA-Z0-9-]+)$/)
  if (guideMatch) return <App key={path} pageSlug={`guide:${guideMatch[1]}`} />
  if (['/shop', '/skin-care', '/hair-care', '/body-care', '/about', '/faq', '/guides', '/contact', '/shipping-returns'].includes(path)) return <App key={path} pageSlug={path.slice(1) as 'shop' | 'skin-care' | 'hair-care' | 'body-care' | 'about' | 'faq' | 'guides' | 'contact' | 'shipping-returns'} />
