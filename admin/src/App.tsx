...
function LegacyShipmentOperations({ order, canManage, onRefresh }: { order: AdminOrder; canManage: boolean; onRefresh: () => void }) {
  ...
  const formatDeliveryDate = (value: unknown) => { const parsed = new Date(String(value ?? '')); return Number.isNaN(parsed.valueOf()) ? null : new Intl.DateTimeFormat('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }).format(parsed) }
  const estimatedFrom = formatDeliveryDate(shipment?.estimatedDeliveryFrom)
  const estimatedTo = formatDeliveryDate(shipment?.estimatedDeliveryTo)
...
type LaunchPromotionSettings = { id: string; enabled: boolean; discountPercent: number; maximumOrders: number; offlineReservations: number; successfulOrders: number; committedReservations: number; startsAt: string; endsAt: string | null; eligibleProductIds: string[]; eligibleCategories: string[]; remainingOrders: number; status: string; message: string }
...
function LaunchPromotionPanel() {
  ...
  const [offlineReservations, setOfflineReservations] = useState('0')
  ...
  const mutation = useMutation({ mutationFn: () => patch<LaunchPromotionSettings>('/admin/launch-promotion', { enabled, discountPercent: Number(discountPercent), maximumOrders: Number(maximumOrders), offlineReservations: Number(offlineReservations), startsAt: startsAt ? new Date(startsAt).toISOString() : undefined, endsAt: endsAt ? new Date(endsAt).toISOString() : null, eligibleProductIds: eligibleProductIds.split(',').map((value) => value.trim()).filter(Boolean), eligibleCategories: eligibleCategories.split(',').map((value) => value.trim()).filter(Boolean) }), onSuccess: (next) => { queryClientForPromotion.invalidateQueries({ queryKey: ['launch-promotion'] }); setEnabled(next.enabled); setDiscountPercent(String(next.discountPercent)); setMaximumOrders(String(next.maximumOrders)); setOfflineReservations(String(next.offlineReservations)); setStartsAt(toDateTimeLocal(next.startsAt)); setEndsAt(next.endsAt ? toDateTimeLocal(next.endsAt) : ''); setEligibleProductIds(next.eligibleProductIds.join(', ')); setEligibleCategories(next.eligibleCategories.join(', ')) } })
  useEffect(() => { if (!query.data) return; setEnabled(query.data.enabled); setDiscountPercent(String(query.data.discountPercent)); setMaximumOrders(String(query.data.maximumOrders)); setOfflineReservations(String(query.data.offlineReservations ?? 0)); setStartsAt(toDateTimeLocal(query.data.startsAt)); setEndsAt(query.data.endsAt ? toDateTimeLocal(query.data.endsAt) : ''); setEligibleProductIds(query.data.eligibleProductIds.join(', ')); setEligibleCategories(query.data.eligibleCategories.join(', ')) }, [query.data])
  const valid = Number.isInteger(Number(discountPercent)) && Number(discountPercent) >= 0 && Number(discountPercent) <= 100 && Number.isInteger(Number(maximumOrders)) && Number(maximumOrders) > 0 && Number.isInteger(Number(offlineReservations)) && Number(offlineReservations) >= 0 && Number(offlineReservations) <= Number(maximumOrders)
  const adjustOfflineReservations = (delta: number) => { const capacity = Math.max(0, Number(maximumOrders) || 0); const current = Math.max(0, Number(offlineReservations) || 0); setOfflineReservations(String(Math.min(capacity, current + delta))) }
  const previewCapacity = Math.max(1, Number(maximumOrders) || query.data?.maximumOrders || 500)
  const parsedPreviewRemaining = Number(previewRemaining)
  ...
  return <section className="panel launch-promotion-panel">
    ...
    <div className="launch-promotion-summary"><div><span>Status</span><strong className={`status-chip status-chip--${query.data?.status ?? 'active'}`}>{humanize(query.data?.status ?? 'active')}</strong></div><div><span>Committed reservations</span><strong>{query.data?.committedReservations ?? ((query.data?.successfulOrders ?? 0) + (query.data?.offlineReservations ?? 0))} / {query.data?.maximumOrders ?? maximumOrders}</strong></div><div><span>Offline confirmed</span><strong>{query.data?.offlineReservations ?? offlineReservations}</strong></div><div><span>{previewMode ? 'Preview remaining' : 'Remaining'}</span><strong>{visibleRemaining}</strong></div></div>
    <form className="launch-promotion-form" onSubmit={(event) => { event.preventDefault(); if (valid) mutation.mutate() }}>
      ...
      {previewMode && <div className="promotion-preview" aria-label="Local promotion preview"><div><span className="kicker">Preview only</span><strong>Launch offer · {discountPercent}% off</strong><small>{previewValue.toLocaleString('en-IN')} reservations left of {previewCapacity.toLocaleString('en-IN')}</small></div><label>Preview remaining reservations<input type="number" min="0" max={previewCapacity} step="1" value={previewRemaining} onChange={(event) => setPreviewRemaining(event.target.value)} placeholder={String(query.data?.remainingOrders ?? previewCapacity)} /></label></div>}
      <div className="form-grid"><label>Discount percentage<input type="number" min="0" max="100" step="1" value={discountPercent} onChange={(event) => setDiscountPercent(event.target.value)} /><small>Applied before tax and shipping only to products whose selling price equals MRP. Existing catalogue markdowns are not discounted twice.</small></label><label>Maximum launch reservations<input type="number" min="1" max="1000000" step="1" value={maximumOrders} onChange={(event) => setMaximumOrders(event.target.value)} /><small>Online orders plus confirmed offline reservations stop at this count.</small></label><label className="field-grid__wide">Offline confirmed reservations<div className="offline-reservation-control"><div><strong>Captured outside checkout</strong><small>Use this for customers confirmed offline. It reduces public capacity after you save.</small></div><div className="offline-reservation-stepper"><button type="button" className="icon-button" onClick={() => adjustOfflineReservations(-1)} disabled={Number(offlineReservations) <= 0} aria-label="Remove one offline reservation"><Minus size={15} /></button><input aria-label="Offline confirmed reservations" type="number" min="0" max={Math.max(0, Number(maximumOrders) || 0)} step="1" value={offlineReservations} onChange={(event) => setOfflineReservations(event.target.value)} /><button type="button" className="icon-button" onClick={() => adjustOfflineReservations(1)} disabled={Number(offlineReservations) >= (Number(maximumOrders) || 0)} aria-label="Add one offline reservation"><Plus size={15} /></button></div></div><small>These are separate from paid online orders; no fake order or payment record is created.</small></label><label>Starts at<input type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} /></label><label>Ends at <span className="optional">optional</span><input type="datetime-local" value={endsAt} onChange={(event) => setEndsAt(event.target.value)} /></label><label className="field-grid__wide">Eligible product IDs <span className="optional">optional</span><input value={eligibleProductIds} onChange={(event) => setEligibleProductIds(event.target.value)} placeholder="Leave blank for every product" /><small>Comma-separated database product IDs. Leave blank to include every product.</small></label><label className="field-grid__wide">Eligible categories <span className="optional">optional</span><input value={eligibleCategories} onChange={(event) => setEligibleCategories(event.target.value)} placeholder="e.g. skin, body" /><small>Comma-separated category values. Both filters must match when provided.</small></label></div>
      <p className="fine-print">Customer message: “{query.data?.message ?? `Exclusive launch access — enjoy ${discountPercent}% off for the first ${maximumOrders} reservations.`}”</p>
