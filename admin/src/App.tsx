...
function View({ active, role }: { active: string; role: AdminRole }) { if (active === 'dashboard') return <DashboardOverview role={role} />; if (active === 'products') return <Products />; if (active === 'inventory') return <Inventory canManage={role === 'SUPER_ADMIN' || role === 'CATALOG_MANAGER'} />; if (active === 'orders') return <Orders canManage={role === 'SUPER_ADMIN' || role === 'ORDER_MANAGER'} />; if (active === 'waitlist') return <WaitlistManagement canManage={role === 'SUPER_ADMIN' || role === 'ORDER_MANAGER'} />; if (active === 'customers') return <Customers canManage={role === 'SUPER_ADMIN' || role === 'SUPPORT_AGENT'} canAnonymize={role === 'SUPER_ADMIN'} />; if (active === 'affiliates') return <Affiliates />; if (active === 'users') return <UsersView />; if (active === 'care-finder') return <CareFinder />; if (active === 'settings') return <SettingsView />; const config = resourceConfig[active]; return config ? <ResourceView {...config} /> : <DashboardOverview role={role} /> }
