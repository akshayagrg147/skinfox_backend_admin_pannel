...
function Products() {
  ...
  const query = useQuery({ queryKey: ['products'], queryFn: () => get<Resource[]>('/admin/products?limit=100') })
  if (editor) return <ProductEditor id={editor === 'new' ? undefined : editor} onBack={() => setEditor(null)} />
  const bySlug = new Map((query.data ?? []).map((product) => [String(product.slug ?? ''), product]))
  const approved = fixedCatalog.map((item) => bySlug.get(item.slug)).filter((item): item is Resource => Boolean(item))
  const additional = (query.data ?? []).filter((product) => !fixedCatalogBySlug.has(String(product.slug))).sort((a, b) => String(a.name ?? '').localeCompare(String(b.name ?? '')))
  const products = [...approved, ...additional]
  return <><div className="page-intro"><div><span className="kicker">SkinFox catalogue</span><h2>Products</h2><p className="muted">The original seven use locked artwork. New products use image files already bundled in the website’s <code>public/products</code> assets.</p></div><div className="page-actions"><button className="secondary-button" onClick={() => query.refetch()}>Refresh <Activity size={16} /></button><button className="primary-button" onClick={() => setEditor('new')}><Plus size={17} /> New product</button></div></div>{query.isLoading ? <TableSkeleton /> : query.isError ? <ErrorPanel onRetry={() => query.refetch()} /> : <div className="catalog-grid">{products.map((product) => { const visual = fixedCatalogBySlug.get(String(product.slug)); const assetPath = visual?.image ?? String(product.image ?? ''); const assetAlt = visual?.alt ?? String(product.imageAlt ?? `${String(product.name ?? 'SkinFox product')} product image`); const hasOffer = Number(product.pricePaise ?? 0) > 0; const hasMrp = Number(product.mrpPaise ?? 0) > 0; return <article className="admin-product-card" key={product.id}><div className="admin-product-card__visual">{assetPath ? <img src={productAssetUrl(assetPath)} alt={assetAlt} /> : <ImageIcon aria-hidden="true" />}</div><div className="admin-product-card__body"><div className="admin-product-card__eyebrow"><span>{String(product.category ?? 'SkinFox')}</span><span className={`catalog-status catalog-status--${String(product.status ?? 'draft')}`}>{String(product.status ?? 'draft').replaceAll('_', ' ')}</span></div><h3>{String(product.name ?? 'Product')}</h3><p>{String(product.subtitle ?? '')}</p><div className="admin-product-card__meta"><span>{String(product.size ?? 'Size not set')}</span><span><em>Offer price</em><strong>{hasOffer ? formatPaise(product.pricePaise) : 'Not set'}</strong>{hasMrp && <small className={hasOffer && Number(product.mrpPaise) > Number(product.pricePaise) ? 'is-discounted' : ''}>MRP {formatPaise(product.mrpPaise)}</small>}</span></div><button className="secondary-button" onClick={() => setEditor(product.id)}>Edit product <ChevronRight size={16} /></button></div></article> })}</div>}</>
}
